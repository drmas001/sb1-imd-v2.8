...
const safetyTypes = [
  ...
  { id: 'observation', label: 'Observation', color: 'yellow' },
  { id: 'short-stay', label: 'Short Stay', color: 'green' }
];
...
const AdmissionForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Reset weekend shift if not weekend
    if (!isWeekendDay && formData.weekendShift) {
    ...
    try {
      ...
      const patientData = {
        ...
        admission: {
          ...
          shift_type: isWeekend ? formData.weekendShift : formData.shiftType,
        ...
        <div>
          ...
          {isWeekend && (
            <p className="mt-1 text-sm text-yellow-600">
              Weekend admission - Please select a weekend shift
            </p>
          )}
        </div>
            ...
            {formData.safetyType && (
              <div className="absolute right-3 top-1/2 -translate-y-1/2">
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                  formData.safetyType === 'emergency'
                    ? 'bg-red-100 text-red-800'
                    : formData.safetyType === 'observation'
